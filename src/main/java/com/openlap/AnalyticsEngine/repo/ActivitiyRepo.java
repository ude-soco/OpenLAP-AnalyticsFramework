...
public interface ActivitiyRepo extends MongoRepository<Activitiy, String> {
	...
	List<Activitiy> findContextualidbyactivitytype(ObjectId organizationId, ObjectId lrsId,
																  String type);

	@Query(value = "{'$and':[{'organisation':?0},{'lrs_id':?1}]}", 	fields = "{ '?2.?3':1}")
	List<Activitiy> findkeysbyContextualidandactivitytype(ObjectId organizationId, ObjectId lrsId, String extension, String extensionId);
